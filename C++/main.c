...
#define PORT 8081
    ...
    void handle_put(int new_socket,char pRequest[]);
    ...
    void handle_post(int new_socket, char pRequest[]) {
        ...
        file = fopen(file_path, "a"); //w para sobre escribir, a para agregar al final
        if (file == NULL) {
	...
	printf("pRequest is %s", pRequest);
        // Formar la respuesta HTTP
        format_response(response_buffer, "200 OK", pRequest, strlen(file_response), file_response);
	...
	char confirmation_message[15];
        ...
        if (file == NULL) {
            printf("No se pudo abrir el archivo JSON para escribir");
            exit(EXIT_FAILURE);
        }
        ...
        // Formar la respuesta HTTP
        format_response(response_buffer, confirmation_message, pRequest, strlen(file_response), file_response);
